...
export default function slackin({
	...
	// splash page
	app.get('/', (req, res) => {
		let name = 'The Society for Excellence in Software and Design';
		let logo = '/assets/ansible.png';
		let { active, total } = slack.users;

		if (!name) return res.send(404);
		let page = dom('html',
			dom('head',
				dom('title', 'Join The Society for Excellence in Software and Design on Slack!'),
				dom('meta name=viewport content="width=device-width,initial-scale=1.0,minimum-scale=1.0,user-scalable=no"'),
				dom('<link href="https://fonts.googleapis.com/css?family=Crimson+Text|Lora" rel="stylesheet">'),
			...
			splash({ name, logo, channels, active, total })
		);
